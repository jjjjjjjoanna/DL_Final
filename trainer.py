...
class Trainer(nn.Module):
    def __init__(self, config):
        ...
        self.classifier = VGG()
        self.bmi_vit = FaceToBMIVIT().to(self.device)
        # Optimizers
        ...
        self.gen_opt = torch.optim.Adam(self.gen_params, lr=config['lr'], betas=(
            config['beta_1'], config['beta_2']), weight_decay=config['weight_decay'])
        self.dis_opt = torch.optim.Adam(self.dis_params, lr=config['lr'], betas=(
            config['beta_1'], config['beta_2']), weight_decay=config['weight_decay'])
        self.gen_scheduler = torch.optim.lr_scheduler.StepLR(
            self.gen_opt, step_size=config['step_size'], gamma=config['gamma'])
    ...
    def initialize(self, vgg_dir, bmi_dir):
        ...
        self.bmi_vit.load_state_dict(
            torch.load(bmi_dir, map_location=self.device))
    ...
    def BMI_loss(self, x_a, x_a_modif):
        ...
        x_a_vit = self.vit_transforms(x_a)
        x_a_vit = x_a_vit.unsqueeze(0)
        x_a_vit = x_a_vit.to(self.device)

        x_a_modif_vit = self.vit_transforms(x_a_modif)
        x_a_modif_vit = x_a_modif_vit.unsqueeze(0)
        x_a_modif_vit = x_a_modif_vit.to(self.device)

        with torch.no_grad():
            org_pred = self.bmi_vit(x_a_vit).item()
            modif_pred = self.bmi_vit(x_a_modif_vit).item()
        return (modif_pred - org_pred) ** 2

    def grad_penalty_r1(self, net, x, coeff=10):
        """Calculate R1 regularization gradient penalty"""
        x.requires_grad = True
        real_predict = net(x)
        gradients = grad(outputs=real_predict.mean(),
    ...
    def gen_encode(self, x_a, age_a, age_b=0, training=False, target_age=0):
        ...
        x_a_recon = self.dec(self.content_code_a,
                             style_params_a, skip_1, skip_2)
        x_a_modif = self.dec(self.content_code_a,
                             style_params_b, skip_1, skip_2)

        return x_a_recon, x_a_modif, age_modif

    def compute_gen_loss(self, x_a, x_b, age_a, age_b, log=False):
        # Generate modified image
        x_a_recon, x_a_modif, age_a_modif = self.gen_encode(
            x_a, age_a, age_b, training=True)
        ...
        self.age_diff = torch.mean(
            torch.abs(predict_age - age_a_modif.float()))
    ...
    def compute_dis_loss(self, x_a, x_b, age_a, age_b):
        ...
        self.loss_dis = self.GAN_loss(self.realism_b, True).mean(
        ) + self.GAN_loss(self.realism_a_modif, False).mean()

        self.loss_dis_gp = self.config['w']['dis'] * \
            self.loss_dis + self.config['w']['gp']*self.loss_gp
    ...
    def log_image(self, x_a, age_a, logger, n_epoch, n_iter):
        ...
        logger.add_images('epoch'+str(n_epoch+1)+'/iter'+str(n_iter+1)+'/content_modif_' +
                          str(age_a_modif.cpu().numpy()[0]), clip_img(x_a_modif), n_iter + 1)

    def log_loss(self, logger, n_iter):
        logger.add_scalar('loss/total', self.loss_gen.item() +
                          self.loss_dis_gp.item(), n_iter + 1)
        ...
        logger.add_scalar('dis/realism_A_modif',
                          self.realism_a_modif.mean().item(), n_iter + 1)
        logger.add_scalar(
            'dis/realism_B', self.realism_b.mean().item(), n_iter + 1)
    ...
    def save_model(self, log_dir):
        ...
        torch.save(self.mlp_style.state_dict(),
                   '{:s}/mlp_style.pth.tar'.format(log_dir))
    ...
    def load_model(self, log_dir):
        self.enc.load_state_dict(torch.load(
            '{:s}/enc.pth.tar'.format(log_dir)))
        self.mlp_style.load_state_dict(torch.load(
            '{:s}/mlp_style.pth.tar'.format(log_dir)))
        self.dis.load_state_dict(torch.load(
            '{:s}/dis.pth.tar'.format(log_dir)))
        self.dec.load_state_dict(torch.load(
            '{:s}/dec.pth.tar'.format(log_dir)))
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        self.gen_scheduler.load_state_dict(
            state_dict['gen_scheduler_state_dict'])
        self.dis_scheduler.load_state_dict(
            state_dict['dis_scheduler_state_dict'])
